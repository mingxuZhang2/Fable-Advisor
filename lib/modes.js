...
Severity — apply strictly, never inflate: critical = wrong results, data loss, security hole, or crash on realistic inputs with no unstated assumptions; major = real defect that fires under specific conditions (name them); minor = worth fixing, will not break anything today. For research code, grade by whether the flaw can change the paper's conclusions — a weakness that cannot affect the claims is minor at most, and say so.
Report every finding the author would genuinely fix — do not stop at the first, and do not pad to look thorough. Do not flag: missing docstrings/comments/type annotations, naming or style preferences, broader-vs-narrower exception types, or intentional design choices — unless they cause a concrete defect. Do not speculate that code "may" break something elsewhere: cite the code that is provably affected, or list it as an open question.
Format each finding: "[severity] file:line — title", then what is wrong, the triggering scenario, and the concrete fix.
Do not comment on macro architecture unless it directly causes a correctness bug. End with a one-paragraph verdict: safe to rely on as-is? yes / no / with fixes.`,
...
Sweep at minimum: input validation/injection, authn/authz, secrets handling, concurrency/races, resource and error handling, data integrity, dependency risk — plus anything the prompt adds.
...
Lay out the viable options with honest trade-offs, then give ONE clear recommendation and the reasoning behind it. Rank options by risk and reversibility; never give time/effort estimates. Sketches and pseudo-code over full diffs.
...
Workflow: first understand the goal and the claims being made; read the code and configs enough to know what was ACTUALLY run (not what the README says); read the results; only then interpret. For large repos or many result files, spawn subagents (Agent tool) to read in parallel and synthesize their reports yourself.
